...
router.post("/progress/batch", async (req, res) => {
  ...
  try {
    ...
    for (const candidateId of candidateIds) {
      ...
      // 1. Check candidate's isInProgress flag from Candidate_Profile
      const candidateResult = await session.run(
      ...
      if (candidateResult.records.length > 0) {
        isInProgress = candidateResult.records[0].get('isInProgress') === true;
      }
      
      // 2. If demandId is provided, also check the status in selected-candidates
      if (demandId && !isInProgress) {
        const selectedResult = await session.run(
          `MATCH (sc:SelectedCandidate {canId: $candidateId, demandId: $demandId})
           RETURN sc.status as status`,
          { candidateId: parseInt(candidateId), demandId: parseInt(demandId) }
        );
        
        if (selectedResult.records.length > 0) {
          status = selectedResult.records[0].get('status');
          // If status is in activeStatuses, mark as in progress
          if (status && activeStatuses.includes(status)) {
            isInProgress = true;
            console.log(`   Candidate ${candidateId}: status "${status}" -> In Progress = true`);
          }
        }
      }
