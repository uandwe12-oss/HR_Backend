...
/**
 * PUT /api/selected-candidates/:demandId/update-progress
 * Update all candidates' isInProgress flag to false for a demand
 */
router.put("/:demandId/update-progress", async (req, res) => {
  const { demandId } = req.params;
  
  console.log(`\n📡 PUT /api/selected-candidates/${demandId}/update-progress - Updating all candidates to not in progress`);
  ...
  try {
    ...
    const result = await session.run(`
      MATCH (d:Demand {id: $demandId})-[r:HAS_SELECTED_CANDIDATE]->(c:Candidate_Profile)
      RETURN c.Can_ID as candidateId
    `, { demandId: parseInt(demandId) });
    
    const candidateIds = result.records.map(record => toNumber(record.get('candidateId')));
    
    console.log(`📊 Found ${candidateIds.length} candidates for demand ${demandId}`);
    
    // Update each candidate's isInProgress flag to false
    for (const candidateId of candidateIds) {
      try {
        // Call the progress endpoint or update directly
        await axios.put(`http://localhost:5000/api/candidates/${candidateId}/progress`, {
          isInProgress: false
        });
        console.log(`✅ Updated candidate ${candidateId} isInProgress to false`);
      } catch (updateErr) {
        console.error(`❌ Failed to update candidate ${candidateId}:`, updateErr.message);
      }
    }
    
    res.json({
      success: true,
      message: `Updated ${candidateIds.length} candidates to not in progress`,
      data: { updatedCount: candidateIds.length }
    });
    
  } catch (err) {
    console.error("❌ Error updating candidates progress:", err);
    res.status(500).json({
      success: false,
      message: "Failed to update candidates progress",
      error: err.message
    });
  } finally {
    await session.close();
 ...
 * Delete all selected candidates for a demand and update isInProgress to false
 */
router.delete("/:demandId", async (req, res) => {
  ...
  try {
    // Get all candidate IDs first (to update their status)
    ...
    console.log(`✅ Removed relationships for ${candidateIds.length} candidates`);
    
    res.json({
      success: true,
      message: `Cleared ${candidateIds.length} selected candidates and updated their status`,
      data: { clearedCount: candidateIds.length }
    });
